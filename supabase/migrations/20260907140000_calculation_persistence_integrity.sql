-- MILLIMETRE V1: calculation persistence integrity
-- Keep the calculation RPC invoker-safe by enforcing canonical project/furniture
-- lineage in the database as well as in the API layer.
...
begin
  if not exists (
    ...
    raise exception 'Calculation run project does not exist';
  end if;

  if not exists (
    select 1
    from public.furniture_items f
    where f.id = new.furniture_item_id
      and f.project_id = new.project_id
  ) then
    raise exception 'Calculation run furniture item must belong to the same project';
  end if;
...
before insert or update of project_id, furniture_item_id
on public.calculation_runs
for each row execute function public.validate_calculation_run_project_lineage();

-- The persistence RPC is SECURITY INVOKER, so callers are still subject to RLS.
-- Add an explicit canonical-lineage guard before creating a run and its cutting list.
...
begin
  ...
  if not exists (
    ...
    raise exception 'Calculation project does not exist';
  end if;

  if not exists (
    select 1
    from public.furniture_items f
    where f.id = p_furniture_item_id
      and f.project_id = p_project_id
  ) then
    raise exception 'Calculation furniture item must belong to the same project';
  end if;
