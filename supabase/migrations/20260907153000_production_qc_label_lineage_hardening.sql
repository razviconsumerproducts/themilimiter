...
begin
  if new.cutting_list_id is not null and not exists (
    select 1 from public.cutting_lists cl
    where cl.id = new.cutting_list_id
      and cl.project_id = new.project_id
      and cl.status in ('APPROVED','RELEASED')
  ) then
    raise exception 'Production order requires an approved or released cutting list belonging to the same project';
  end if;
  return new;
end;
$$;
...
returns trigger language plpgsql as $$
declare order_project uuid; order_cutting_list uuid; item_project uuid; item_run uuid; order_run uuid;
begin
  ...
  if order_project is null then raise exception 'Production piece must belong to a valid production order'; end if;

  if new.cutting_list_item_id is not null then
    select project_id, calculation_run_id into item_project, item_run
    from public.cutting_list_items where id = new.cutting_list_item_id;
    ...
    if order_cutting_list is not null then
      select calculation_run_id into order_run from public.cutting_lists where id = order_cutting_list;
      if order_run is null or item_run <> order_run then
        raise exception 'Production piece cutting list item must belong to the production order cutting list lineage';
      end if;
    end if;
  end if;
  return new;
...
begin
  ...
  ) then raise exception 'Work order piece must belong to the same production order'; end if;
  return new;
end;
$$;
...
begin
  if new.work_order_id is not null and not exists (
    select 1 from public.work_orders wo where wo.id = new.work_order_id and wo.production_order_id = new.production_order_id
  ) then raise exception 'Production material issue work order must belong to the same production order'; end if;
  if new.inventory_transaction_id is not null and not exists (
    select 1 from public.inventory_transactions it
    join public.production_orders po on po.id = new.production_order_id
    where it.id = new.inventory_transaction_id and it.project_id = po.project_id
  ) then raise exception 'Production material issue inventory transaction project mismatch'; end if;
  return new;
end;
$$;
...
before insert or update of production_order_id, work_order_id, inventory_transaction_id on public.production_material_issues
...
begin
  if not exists (
    select 1 from public.production_pieces pp
    where pp.id = new.production_piece_id and pp.production_order_id = new.production_order_id
  ) then raise exception 'Production QC piece must belong to the same production order'; end if;
  if new.work_order_id is not null and not exists (
  ...
  ) then raise exception 'Production QC work order must match the production order and piece'; end if;
  return new;
end;
$$;
...
before insert or update of project_id, production_order_id, production_piece_id, work_order_id on public.production_qc_inspections
...
begin
  if not exists (
    select 1 from public.production_qc_inspections q
    where q.id = new.production_qc_inspection_id
      and q.project_id = new.project_id and q.production_piece_id = new.production_piece_id
  ) then raise exception 'Production QC defect must match its inspection project and piece'; end if;
  return new;
end;
$$;
...
before insert or update of project_id, production_qc_inspection_id, production_piece_id on public.production_qc_defects
...
returns trigger language plpgsql as $$
begin
  if not exists (select 1 from public.production_orders po where po.id = new.production_order_id and po.project_id = new.project_id) then
...
before insert or update of project_id, production_order_id on public.production_release_gates
...
begin
  ...
  if new.production_piece_id is not null and not exists (
    select 1 from public.production_pieces pp where pp.id = new.production_piece_id and pp.production_order_id = new.production_order_id
  ) then raise exception 'Production label piece must belong to the same production order'; end if;
  return new;
end;
$$;
...
before insert or update of project_id, production_order_id, production_piece_id on public.production_labels
...
returns trigger language plpgsql as $$
begin
  if not exists (select 1 from public.production_labels pl where pl.id = new.label_id and pl.project_id = new.project_id) then
...
before insert or update of project_id, label_id on public.label_print_jobs
...
-- Explicitly replace only this migration's policy names; existing access model remains authenticated-only.
