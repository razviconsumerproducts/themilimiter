...
begin
  select cr.project_id into v_run_project
  from public.calculation_runs cr
  where cr.id = new.calculation_run_id;
  ...
  if new.furniture_item_id is not null then
    select fi.project_id into v_furniture_project
    from public.furniture_items fi
    where fi.id = new.furniture_item_id;
  ...
  if new.component_id is not null then
    select fc.furniture_item_id into v_component_furniture
    from public.furniture_components fc
    where fc.id = new.component_id;

    if v_component_furniture is null then
      raise exception 'Cutting list item component does not exist';
    end if;
...
begin
  select cr.project_id, cr.status
    into v_calc_project, v_calc_status
  from public.calculation_runs cr
  where cr.id = new.calculation_run_id;
  ...
  if v_calc_status <> 'valid' then
    raise exception 'BOM requires a valid calculation run';
  end if;
...
declare
  ...
  v_component_furniture uuid;
begin
  select b.project_id into v_bom_project
  from public.boms b
  ...
  if new.source_component_id is not null then
    select fc.furniture_item_id into v_component_furniture
    from public.furniture_components fc
    where fc.id = new.source_component_id;

    if v_component_furniture is null then
      raise exception 'BOM source component does not exist';
    end if;
...
-- -----------------------------------------------------------------------------
-- OPTIMIZATION -> RELEASED CUTTING LIST
-- -----------------------------------------------------------------------------
...
declare
  ...
  v_calc_project uuid;
begin
  select cl.project_id, cl.status, cr.project_id
    into v_cut_project, v_cut_status, v_calc_project
...
begin
  select orr.project_id into v_run_project
  from public.optimization_runs orr
  where orr.id = new.optimization_run_id;

  if v_run_project is null then
    raise exception 'Optimization sheet requires an existing optimization run';
  end if;

  select exists(select 1 from public.materials m where m.id = new.material_id)
...
declare
  ...
  v_cut_project uuid;
begin
  select os.optimization_run_id into v_run_id
  from public.optimization_sheets os
  where os.id = new.optimization_sheet_id;

  if v_run_id is null then
    raise exception 'Optimization placement requires an existing optimization sheet';
  end if;

  select orr.project_id into v_run_project
  from public.optimization_runs orr
  where orr.id = v_run_id;

  select cl.project_id into v_cut_project
  from public.cutting_lists cl
  join public.optimization_runs orr on orr.cutting_list_id = cl.id
  where orr.id = v_run_id;
  ...
  if not exists (
    ...
    join public.optimization_runs orr on orr.cutting_list_id = cli.calculation_run_id::uuid
    where cli.id = new.cutting_list_item_id
  ) then
    -- The placement must belong to the run's cutting list. Resolve directly below;
    -- the cast above is intentionally not relied upon for acceptance.
    if not exists (
      select 1
      from public.cutting_list_items cli
      join public.optimization_runs orr on orr.cutting_list_id in (
        select cl.id from public.cutting_lists cl
        where cl.id = orr.cutting_list_id
      )
      where cli.id = new.cutting_list_item_id
        and orr.id = v_run_id
    ) then
...
-- -----------------------------------------------------------------------------
-- COSTING -> APPROVED BOM + APPROVED OPTIMIZATION
-- -----------------------------------------------------------------------------
...
begin
  if new.bom_id is null then
    raise exception 'Costing run requires a BOM';
  end if;

  select b.project_id, b.status, b.calculation_run_id
    into v_bom_project, v_bom_status, v_bom_calc
  from public.boms b
  where b.id = new.bom_id;
  ...
  if v_bom_status not in ('APPROVED','RELEASED') then
    raise exception 'Costing run requires an approved or released BOM';
  end if;

  if new.optimization_run_id is null then
    raise exception 'Costing run requires an optimization run';
  end if;

  select orr.project_id, orr.status, orr.cutting_list_id
    into v_opt_project, v_opt_status, v_opt_cut
  from public.optimization_runs orr
  where orr.id = new.optimization_run_id;
  ...
  if v_opt_status <> 'APPROVED' then
    raise exception 'Costing run requires an approved optimization run';
  end if;
...
-- Costing items cannot silently point at another project than their costing run.
