...
import { calculateCosting, type CostCategory, type CostingItemInput, type CostingInput } from '../../../../lib/millimetre'

const CATEGORIES: CostCategory[] = ['MATERIAL','HARDWARE','EDGE_BAND','LABOUR','SERVICE','TRANSPORT','INSTALLATION','OVERHEAD','OTHER']
const MARGIN_TYPES: NonNullable<CostingInput['marginType']>[] = ['MARKUP_PERCENT','GROSS_MARGIN_PERCENT','FIXED_MARGIN','FIXED_SELLING_PRICE']
...
export async function POST(request: Request) {
  try {
    ...
    const marginType = body.marginType == null || String(body.marginType).trim() === '' ? undefined : String(body.marginType).trim().toUpperCase() as CostingInput['marginType']
    if (marginType && !MARGIN_TYPES.includes(marginType)) throw new Error('Invalid marginType.')
    const marginValue = nonNegative(body.marginValue, 'marginValue')
    const result = calculateCosting({ projectId, currency: body.currency == null ? 'INR' : required(body.currency, 'currency'), items, discount: nonNegative(body.discount, 'discount'), taxRate: nonNegative(body.taxRate, 'taxRate'), marginType, marginValue })
    const inputSnapshot = { projectId, calculationRunId, optimizationRunId, version, items, currency: result.currency, discount: result.discount, taxRate: body.taxRate ?? 0, marginType: marginType ?? null, marginValue, requestedBy: user.id }
    const { data, error } = await supabase.rpc('persist_costing_run', { p_project_id: projectId, p_calculation_run_id: calculationRunId, p_optimization_run_id: optimizationRunId, p_currency: result.currency, p_material_cost: result.items.filter(i => i.category === 'MATERIAL').reduce((s,i) => s+i.totalCost,0), p_edge_banding_cost: result.items.filter(i => i.category === 'EDGE_BAND').reduce((s,i) => s+i.totalCost,0), p_hardware_cost: result.items.filter(i => i.category === 'HARDWARE').reduce((s,i) => s+i.totalCost,0), p_manufacturing_cost: result.items.filter(i => ['LABOUR','SERVICE','TRANSPORT','INSTALLATION'].includes(i.category)).reduce((s,i) => s+i.totalCost,0), p_wastage_cost: result.items.reduce((s,i) => s+i.wastageCost,0), p_overhead_cost: result.items.filter(i => i.category === 'OVERHEAD').reduce((s,i) => s+i.totalCost,0), p_total_cost: result.totalCost, p_status: 'calculated', p_result: { ...result, inputSnapshot }, p_items: result.items.map(i => ({ ...i, unitRate: i.unitCost })) })
