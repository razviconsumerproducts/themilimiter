...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as {
      ...
      items?: Array<{
        itemType: string; sourceType?: string; sourceId?: string; itemCode?: string
        description: string; quantity: number; unit: string; unitPrice: number
        discount?: number; taxRate?: number; notes?: string
      }>
    }

    if (!body.projectId || !body.costingRunId || !body.quotationCode) {
      return NextResponse.json({ error: 'projectId, costingRunId and quotationCode are required.' }, { status: 400 })
    }

    const { data: costing, error: costingError } = await supabase
      .from('costing_runs')
      .select('id, project_id, status, currency, output_snapshot, total_cost, selling_price')
      .eq('id', body.costingRunId)
      .maybeSingle()
    ...
    if (!['APPROVED', 'LOCKED'].includes(costing.status)) {
      return NextResponse.json({ error: 'Quotation requires an approved or locked costing run.' }, { status: 409 })
    }
    ...
    const { data: inserted, error: insertError } = await supabase
      ...
      .insert({
        ...
        costing_run_id: quotation.costingRunId,
        quotation_code: quotation.quotationCode,
