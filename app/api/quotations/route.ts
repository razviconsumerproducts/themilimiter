...
export async function POST(request: Request) {
  try {
    ...
    if (!Number.isInteger(version) || version < 1) return NextResponse.json({ error: 'version must be a positive integer.' }, { status: 400 })

    const items = body.items ?? []
    if (!items.length) return NextResponse.json({ error: 'At least one quotation item is required.' }, { status: 400 })
    ...
    const quotation = buildQuotation({
      projectId: project.id, customerId: project.customer_id, costingRunId: costing.id,
      quotationCode: body.quotationCode, version, currency, items,
      paymentTerms: body.paymentTerms, deliveryTerms: body.deliveryTerms,
      installationTerms: body.installationTerms, warrantyTerms: body.warrantyTerms,
      notes: body.notes, customerNotes: body.customerNotes,
      costSnapshot: costing.output_snapshot ?? { costingRunId: costing.id, totalCost: costing.total_cost, sellingPrice: costing.selling_price },
    })

    const sellingPrice = Number(costing.selling_price)
    if (!Number.isFinite(sellingPrice) || sellingPrice < 0) return NextResponse.json({ error: 'Approved costing has an invalid selling price.' }, { status: 409 })
    if (!Number.isFinite(Number(quotation.grandTotal)) || Math.abs(Number(quotation.grandTotal) - sellingPrice) > 0.02) return NextResponse.json({ error: 'Quotation grand total must reconcile to the approved costing selling price.' }, { status: 409 })

    const quotationDate = new Date().toISOString().slice(0, 10)
    if (body.validUntil && body.validUntil < quotationDate) return NextResponse.json({ error: 'validUntil cannot be before quotation date.' }, { status: 400 })

    const { data: inserted, error: insertError } = await supabase.from('quotations').insert({
      project_id: quotation.projectId, customer_id: quotation.customerId, costing_run_id: quotation.costingRunId,
      quotation_code: quotation.quotationCode, version: quotation.version, status: 'DRAFT', quotation_date: quotationDate,
      valid_until: body.validUntil ?? null, currency: quotation.currency, subtotal: quotation.subtotal, discount: quotation.discount,
      taxable_amount: quotation.taxableAmount, tax_amount: quotation.taxAmount,
      payment_terms: body.paymentTerms ?? null, delivery_terms: body.deliveryTerms ?? null,
      installation_terms: body.installationTerms ?? null, warranty_terms: body.warrantyTerms ?? null,
      notes: body.notes ?? null, customer_notes: body.customerNotes ?? null,
      cost_snapshot: quotation.costSnapshot,
      commercial_snapshot: { ...quotation.commercialSnapshot, source: 'APPROVED_COSTING', costing_run_id: costing.id, costing_version: costing.version, costing_selling_price: sellingPrice },
      created_by: user.id,
    }).select('*').single()
    ...
    const { error: itemError } = await supabase.from('quotation_items').insert(quotation.items.map((item, index) => ({
      ...
      line_total: item.lineTotal, sort_order: index, notes: item.notes ?? null,
    })))
    if (itemError) {
