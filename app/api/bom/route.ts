...
type CalculationPart = {
  id?: string; furnitureId?: string; kind?: string; name?: string; qty?: number
  length?: number; width?: number; thickness?: number; materialId?: string
  edge?: string; edgeBandMm?: number; notes?: string
}
...
function deriveItemsFromCalculation(result: unknown): BomItemInput[] {
  ...
  for (const part of parts) {
    ...
    items.push({
      furnitureItemId: part.furnitureId,
      itemType: part.kind === 'edge_strip' ? 'EDGE_BAND' : 'BOARD',
      itemId: part.materialId,
      itemCode: part.materialId,
      description,
      quantity: qty,
      unit: 'PCS',
      calculationBasis: `Calculation ${String(snapshot.furnitureId ?? '')}; ${dimensions}`,
      sourceComponentId: part.id,
      notes: part.edge && part.edge !== 'none' ? `Edge: ${part.edge}${part.edgeBandMm ? `; ${part.edgeBandMm} mm band` : ''}` : part.notes,
    })
  }
...
export async function POST(request: Request) {
  try {
    ...
    const { data: run, error: runError } = await supabase
      ...
      .select('id,project_id,furniture_item_id,status,output_snapshot,result')
    ...
    if (rawItems.length) {
      ...
      items = deriveItemsFromCalculation(run.output_snapshot ?? run.result)
    }
