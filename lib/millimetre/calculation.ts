...
function part(furnitureId: string, material: Material, kind: Component['kind'], name: string, qty: number, length: number, width: number, edge: Component['edge'] = 'none', grain = false): CuttingPart {
  ...
  return {
    ...
    furnitureId, kind, name, qty: safeQty, length: safeLength, width: safeWidth,
    thickness: material.thickness, materialId: material.id, grain, edge,
...
export function calculateFurniture(input: CalculationInput): CalculationResult {
  ...
  if (!validation.valid) {
    throw new Error(`Calculation validation failed: ${validation.errors.map((e) => e.message).join(' ')}`);
  }

  const { furniture: f, carcassMaterial: carcass, backMaterial: back, shutterMaterial: shutter } = input;
  const t = f.carcassThickness;
  const innerWidth = f.width - 2 * t;
  const innerHeight = f.height - 2 * t;
  ...
  if (input.includeShelves && f.shelfCount) {
    parts.push(part(f.id, carcass, 'shelf', 'Adjustable shelf', f.shelfCount, innerWidth, Math.max(0, f.depth - 20), 'front'));
  }

  if (input.includeDrawers && f.drawerCount) {
    const drawers = f.drawerCount;
    const drawerHeight = Math.floor((innerHeight - Math.max(0, drawers - 1) * 2) / drawers);
  ...
  if (input.includeBack && back) parts.push(part(f.id, back, 'back', 'Back', 1, f.height, f.width));

  if (input.includeShutters && shutter) {
    const gap = f.shutterGap;
    parts.push(part(f.id, shutter, 'shutter', 'Shutter', 1, f.height - gap * 2, f.width - gap * 2, 'all', true));
  }
